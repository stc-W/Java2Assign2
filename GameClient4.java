...
public class GameClient4 extends Application {
    Scanner in;
    PrintWriter out;
    int player;
    int selectX;
    ...
    public void start(Stage stage) throws Exception {
        ...
        gameInterface.chessBoard.setOnMouseClicked(e -> {
            if (e.getX() > 48 && e.getX() < 198 && e.getY() > 2 && e.getY()
              < 152 && currentPlayer[0] == player && start) {
    ...
    public void setChess(int x, int y, int[] currentPlayer) {
        ...
        double x2 = 48 + (x + 1) * 51;
        double y2 = 2 + (y + 1) * 51;
        if (currentPlayer[0] == 1 && !gameInterface.gameOver
          && gameInterface.chessBoard.chessBoard[x][y] == 0) {
